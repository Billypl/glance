...
func TestWidgetContentResponseContainsDataNextUpdate(t *testing.T) {
	app := newTestApp()
	...
	body := w.Body.String()
	if !strings.Contains(body, "data-next-update=") {
		t.Errorf("response missing data-next-update attribute; body: %s", body)
	}
}
